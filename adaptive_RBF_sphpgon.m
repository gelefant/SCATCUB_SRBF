function [I,Ierr,flag,Ihigh,iters,tri_vertices,tri_conn_list,L1_vertices]=...
    adaptive_RBF_sphpgon(vertices,coeff,phi,h,centers,atol,rtol)
...
%--------------------------------------------------------------------------
% Output:
% I      : approximation of integral of a rbf over the spherical triangle
%          defined by vertices
%--------------------------------------------------------------------------
% Subroutines:
...
for k=1:size(tri_conn_list,1)
    ...
    [L1_vertices,L1_integrals,L1_errors,L2_data,Itemp_low,Itemp_high]=...
        start_up(vertices,coeff,phi,h,centers,L1_vertices,L1_integrals,L1_errors,L2_data);
end
...
while  sum(L1_errors) > max([atol rtol*I])
    ...
    for j=1:4
        ...
        L2_data_temp_sons=generate_sphtri_sons(L2_data_temp.vertices,coeff,phi,h,centers);
...
function [L1_vertices,L1_integrals,L1_errors,L2_data,Itemp_low,...
    Itemp_high]=start_up(vertices,coeff,phi,h,centers,L1_vertices,L1_integrals,L1_errors,...
    L2_data)
...
ff = [phi(h,xyzw(:,1:3)*centers')]*coeff;
...
L2_data_temp=generate_sphtri_sons(vertices,coeff,phi,h,centers);
...
function L2_data=generate_sphtri_sons(vertices,coeff,phi,h,centers)
...
L2_data{1}=make_L2_data(vertices,coeff,phi,h,centers);
L2_data{2}=make_L2_data([OAB_mid; OB; OBC_mid],coeff,phi,h,centers);
L2_data{3}=make_L2_data([OBC_mid; OC; OAC_mid],coeff,phi,h,centers);
L2_data{4}=make_L2_data([OAB_mid; OBC_mid; OAC_mid],coeff,phi,h,centers);
...
function L2_dataL=make_L2_data(vertices,coeff,phi,h,centers)
